...
xr_reader *xr_file_system::r_open(const char *path) const
{
	...
	try
	{
		size_t file_size = static_cast<size_t>(fs::file_size(fs::path(path)));
		...
		{ // 1MB threshold
			...
			return new xr_temp_reader(buffer.data(), file_size);
		}
...
// Для больших файлов используем mmap
		...
		HANDLE h_mmap = CreateFileMapping(h, NULL, PAGE_READONLY, 0, file_size, NULL);
		if (h_mmap == NULL)
		{
			CloseHandle(h);
			return nullptr;
		...
		void *data = MapViewOfFile(h_mmap, FILE_MAP_READ, 0, 0, file_size);
		if (data == NULL)
		{
			CloseHandle(h_mmap);
			CloseHandle(h);
		...
		return new xr_mmap_reader(h, h_mmap, data, file_size);
#else
		...
		return new xr_mmap_reader(fd, data, file_size);
#endif
	}
...
xr_writer *xr_file_system::w_open(const char *path, bool ignore_ro) const
{
	...
	try
	{
		...
		return new xr_file_writer(path);
	}
	catch (const std::exception &)
	{
		return nullptr;
...
size_t xr_file_system::file_length(const char *path)
{
	...
	return static_cast<size_t>(fs::file_size(fs::path(path)));
}
...
bool xr_file_system::copy_file(const char *src_path, const char *tgt_path) const
{
	...
	try
	{
		...
		fs::path target_path(tgt_path);
		if (auto parent = target_path.parent_path(); !parent.empty())
		{
			fs::create_directories(parent);
		}

		fs::copy_file(fs::path(src_path), fs::path(tgt_path), fs::copy_options::overwrite_existing);
		return true;
	}
	catch (const std::exception &)
	{
